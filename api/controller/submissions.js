...
async function AllSubmission(req,res) {
	...
	let lastestPromise = new Promise((resolve,reject) => {
			...
			db.query(sql,[userData.id],(err,result) => err ? reject(err) :resolve(result[0]))
	} )
    let submitPromise = new Promise((resolve, reject) => {
		var sql = ''
		if(mode == 'full') sql =
			`SELECT idResult,U.sname,U.rating,U.state,P.name,P.state as see,P.sname as problemname,result,timeuse,Result.score,errmsg FROM Result 
